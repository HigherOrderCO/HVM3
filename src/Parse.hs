...
-- TASK: update the parser to the new version of Core.hs now
...
parseElim = do
  ...
  next <- lookAhead anyChar
  case next of
    '0' -> parseSwiElim val mov
    '(' -> do
      next2 <- lookAhead (anyChar >> anyChar)
      case next2 of
        ',' -> parseGetElim val mov
        _   -> parseUseElim val mov
    '}' -> parseEfqElim val mov
    _   -> fail "Invalid eliminator pattern"

-- Efq: `~x{}`
parseEfqElim :: Core -> [(String, Core)] -> ParserM Core
parseEfqElim val mov = do
  consume "}"
  return $ Efq val mov

-- Use: `~x{():f}`
parseUseElim :: Core -> [(String, Core)] -> ParserM Core
parseUseElim val mov = do
  consume "():"
  body <- parseCore
  consume "}"
  return $ Use val mov body

-- Get: `~x{(,):f}`
parseGetElim :: Core -> [(String, Core)] -> ParserM Core
parseGetElim val mov = do
  consume "(,):"
  body <- parseCore
  consume "}"
  return $ Get val mov body

-- Swi: `~x{0:z;+:s}`
parseSwiElim :: Core -> [(String, Core)] -> ParserM Core
parseSwiElim val mov = do
  consume "0:"
  zero <- parseCore
  consume ";"
  consume "+:"
  succ <- parseCore
  consume "}"
  return $ Swi val mov zero succ

-- Mov: `!m0 = v0` (used inside Elim)
parseMove :: ParserM (String, Core)
parseMove = do
  try $ skip >> consume "!"
  name <- parseName1
...
-- Emp: `⊥`
parseEmp :: ParserM Core
parseEmp = do
  consume "⊥"
  return Emp

-- Uni: `⊤`
parseUni :: ParserM Core
parseUni = do
  consume "⊤"
  return Uni
...
parseBook = do
  skip
  defs <- many $ do
    def <- choice [parseTopImp, parseTopDef]
    skip
    return def
  try $ skip >> eof
  return $ concat defs
...
-- Parse Book and Core
doParseBook filePath code = do
  result <- runParserT p (ParserState MS.empty MS.empty MS.empty 0) "" code
  case result of
    Right (defs, st) -> do
      return $ createBook defs (pFreshLab st)
    Left err -> do
      showParseError filePath code err
      return $ Book MS.empty MS.empty MS.empty MS.empty 0
  where
    p = do
      defs <- parseBook
      st <- getState
...
doParseCore :: String -> IO Core
doParseCore code = do
  result <- runParserT parseCore (ParserState MS.empty MS.empty MS.empty 0) "" code
  case result of
    Right core -> return core
    Left err -> do
      showParseError "" code err
...
-- Book creation and setup functions
createBook :: [(String, ((Bool,[(Bool,String)]), Core))] -> Lab -> Book
createBook defs freshLab =
  let withPrims = \n2i -> MS.union n2i (MS.fromList primitives)
      nameList  = zip (map fst defs) [0..] :: [(String, Word16)]
      namToFid' = withPrims (MS.fromList nameList)
      fidToNam' = MS.fromList (map (\(k,v) -> (v,k)) (MS.toList namToFid'))
...
bindVars vars parse = do
  ...
  let prev = bindDups st
  -- Split into scopeless vars (starting with $) and regular vars
  let (svars, rvars) = partition (\v -> not (null v) && head v == '$') vars
  ...
  where
    ...
    applyDups usages body var = do
      let (uses, dups) = mget usages (stripName var)
      if (not (null var) && head var /= '&') && (uses > 1) then
        fail $ "Linear variable " ++ show var ++ " used " ++ show uses ++ " times"
      else
        return (dups body)
...
makeVar name = do
  st <- getState
  case (if null name then ' ' else head name, MS.lookup name (bindDups st)) of
    ('$', Nothing) -> do -- $-vars can be used before definition
      putState st {bindDups = MS.insert name (1, (\x -> x)) (bindDups st)}
      return $ Var name
    (_, Nothing) -> do
...
setRefIds fids term = case term of
  ...
  Set             -> Set
  Emp             -> Emp
  Efq c ms        -> Efq (setRefIds fids c) (map (\(k,v) -> (k, setRefIds fids v)) ms)
  Uni             -> Uni
  Nil             -> Nil
  Use c ms b      -> Use (setRefIds fids c) (map (\(k,v) -> (k, setRefIds fids v)) ms) (setRefIds fids b)
  U32             -> U32
  W32 n           -> W32 n
  Swi c ms z s    -> Swi (setRefIds fids c) (map (\(k,v) -> (k, setRefIds fids v)) ms) (setRefIds fids z) (setRefIds fids s)
  Op2 op x y      -> Op2 op (setRefIds fids x) (setRefIds fids y)
  Sig a b         -> Sig (setRefIds fids a) (setRefIds fids b)
  Tup a b         -> Tup (setRefIds fids a) (setRefIds fids b)
  Get c ms b      -> Get (setRefIds fids c) (map (\(k,v) -> (k, setRefIds fids v)) ms) (setRefIds fids b)
  All a b         -> All (setRefIds fids a) (setRefIds fids b)
  Lam x f         -> Lam x (setRefIds fids f)
  App f x         -> App (setRefIds fids f) (setRefIds fids x)
...
collectLabels term = case term of
  ...
  Set                 -> MS.empty
  Emp                 -> MS.empty
  Efq c ms            -> MS.union (collectLabels c) (MS.unions $ map (collectLabels . snd) ms)
  Uni                 -> MS.empty
  Nil                 -> MS.empty
  Use c ms b          -> MS.unions [collectLabels c, MS.unions $ map (collectLabels . snd) ms, collectLabels b]
  U32                 -> MS.empty
  W32 _               -> MS.empty
  Swi c ms z s        -> MS.unions [collectLabels c, MS.unions $ map (collectLabels . snd) ms, collectLabels z, collectLabels s]
  Op2 _ x y           -> MS.union (collectLabels x) (collectLabels y)
  Sig a b             -> MS.union (collectLabels a) (collectLabels b)
  Tup a b             -> MS.union (collectLabels a) (collectLabels b)
  Get c ms b          -> MS.unions [collectLabels c, MS.unions $ map (collectLabels . snd) ms, collectLabels b]
  All a b             -> MS.union (collectLabels a) (collectLabels b)
  Lam _ bod           -> collectLabels bod
  App fun arg         -> MS.union (collectLabels fun) (collectLabels arg)
